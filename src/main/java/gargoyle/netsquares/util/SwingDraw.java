...
public final class SwingDraw {
    ...
    public static void drawStringCentered(Graphics2D g, Rectangle2D bounds, String label) {
    ...
    public static void updateFontSize(Graphics g, Rectangle2D bounds, String label) {
        g.setFont(g.getFont().deriveFont((float) getFitFontSize(g.getFont(), label, (int) bounds.getWidth(), (int) bounds.getHeight(), g)));
    }

    private static int getFitFontSize(Font font, String text, int width, int height, Graphics g) {
