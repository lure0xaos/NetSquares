...
class NSPainter(context: NSContext, background: String, backgroundSuffix: String) {
    ...
    private val fontLCD: Font?
    private val fontZX: Font?
    ...
    private fun paintBackground(g: Graphics2D, background: Image?, bounds: Rectangle2D) {
        val inner: Rectangle2D = Rectangle2D.Double(
            0.0, 0.0,
    ...
    fun getCellBounds(bounds: Rectangle2D, boardSize: Int, x: Int, y: Int): Rectangle2D.Double {
    ...
    private fun paintCellInGame(
        ...
        if (open) {
            ...
            val cellBackground =
                if (score > 0) if (reachable) CELL_POSITIVE_HIGHLIGHT else CELL_POSITIVE else if (reachable) CELL_NEGATIVE_HIGHLIGHT else CELL_NEGATIVE
    ...
    private fun paintInfoBarInGame(
        ...
        val rows = 3
        var row = 0
        run { row = paintInfoBarRowInGame(manager, g, bounds, player.name, fontZX, INFOBAR_PLAYER, row, rows) }
        run {
